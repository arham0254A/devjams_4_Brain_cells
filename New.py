# import numpy as np
# import h5py
# import matplotlib.pyplot as plt # Kept for the internal shape function
...
#     shape: (rows, cols) of the local area.
#     center: (r_center, c_center) of the ellipse.
#     radii: (r_radius, c_radius) for the major and minor axes.
...
#     # Create a smooth, deep center with fading edges using a Gaussian-like function
#     mask = np.exp(-ellipse_dist)
    
#     # Set maximum depth at the center
...
# MATRIX_ROWS = int(ROAD_WIDTH / GRID_RESOLUTION)  # 52 rows
# MATRIX_COLS = int(ROAD_LENGTH / GRID_RESOLUTION) # 400 columns
...
# # Pothole Cluster 1: High Severity (Random Round/Elliptical Shapes)
# # Area around column index 200 to 400
# CLUSTER_AREA_C = (200, 400)
# num_potholes_1 = 25

# for i in range(num_potholes_1):
#     # Randomly determine center and size within the cluster's region and road width
#     r_center = np.random.randint(10, MATRIX_ROWS - 10)
#     c_center = np.random.randint(CLUSTER_AREA_C[0], CLUSTER_AREA_C[1])
    
#     r_radius = np.random.uniform(3, 7) # 0.75m to 1.75m radius (in pixels)
#     c_radius = np.random.uniform(r_radius * 0.8, r_radius * 1.2) # Elliptical variation
    
#     max_depth = np.random.uniform(5.0, 9.0) # Depth 5 to 9 cm
#     rotation = np.random.uniform(0, 180)
    
#     # Calculate local matrix boundaries for the mask
...
#     # Add the depth to the main matrix, ensuring we don't reduce existing depth
#     road_matrix[r_min:r_max, c_min:c_max] = np.maximum(
#         road_matrix[r_min:r_max, c_min:c_max],
#         mask * max_depth
#     )

# # Pothole Cluster 2: Low Severity/Sparse (Random Round/Elliptical Shapes)
# CLUSTER_AREA_C_2 = (1200, 1400)
# num_potholes_2 = 15
...
# OUTPUT_H5_PATH = "chunked_road_pothole_data_realistic.h5"
...
# print(f"✅ Generated and saved realistic road data to {OUTPUT_H5_PATH}")
# print(f"Matrix Dimensions: {MATRIX_ROWS} rows x {MATRIX_COLS} columns")
